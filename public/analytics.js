...
(function () {
  ...
  // ===================== REPEATER METRIC SCATTER (#672 / D, port of upstream
  // Kpa-clawbot/CoreScope PR #1760) =====================
  // Each point is a repeater (or room); the axes are two selectable metrics
  // that /api/nodes already attaches to repeater/room rows: all four #672
  // usefulness axes (bridge_score, coverage_score, redundancy_score,
  // traffic_share_score), the composite usefulness_score, plus relay
  // activity (relay_count_1h/24h) and advert_count. This view only PLOTS
  // those metrics; nothing is computed client-side.
  //
  // Traffic vs. Usefulness: upstream's PR #1760 read
  // `traffic_share_score ?? usefulness_score` into a single "traffic" axis.
  // That was harmless when it was written — at the time usefulness_score
  // was just an alias for traffic_share_score (see the historical note in
  // openapi.go). It is NOT harmless now: enrichNodeUsefulness
  // (cmd/server/usefulness_composite.go) sets traffic_share_score and the
  // real weighted composite usefulness_score
  // (0.30*bridge + 0.25*coverage + 0.25*redundancy + 0.20*traffic) together,
  // atomically, on every repeater/room row it touches — so if
  // traffic_share_score is present, usefulness_score already is too, and
  // vice versa. A "traffic share ?? usefulness" fallback would therefore
  // never actually trigger against a real row; it would only ever fire on a
  // row neither field was ever enriched on, silently mislabeling a missing
  // value as a fake "traffic share" reading of the composite. There is no
  // dead-server-version scenario where this fallback would help, so it is
  ...
  async function renderRepeaterMetricsTab(el) {
    ...
    try {
      ...
      // Restore the last-picked axes (default Traffic share x Usefulness). A
  ...
  async function renderDistanceTab(el) {
    try {
      ...
      // #1735-view-path-followup: this used to have its own "View on map"
      // icon button (dist-map-hop, dropped a 2-point route on the main Map
      // page) alongside View Path. Two 48x48 touch-target buttons (the
      // AGENTS glove-operability floor -- can't shrink below that) crammed
      // into a dense 20-row leaderboard was consistently unreadable no
      // matter how the actions cell was positioned. View Path is a strict
      // superset of what View on map showed (same route, plus elapsed
      // time, area shading, branch legend), so the map button is gone --
      // View Path is now the only action, and fits cleanly with no scroll
      // wrapper or sticky-column tricks needed.
      ...
      top20.forEach((h, i) => {
        ...
        html += `<tr title="${esc(tsTitle)}"><td>${i+1}</td><td>${fromLink}</td><td>${toLink}</td><td><strong>${formatDistance(h.dist)}</strong></td><td>${esc(h.type)}</td><td>${obs}</td><td>${bestSnr}</td><td>${medianSnr}</td><td>${pktLink}</td><td>${viewPathBtn}</td></tr>`;
      });
      html += `</tbody></table></div>`;
      ...
      if (data.topPaths.length) {
        ...
        data.topPaths.slice(0, 10).forEach((p, i) => {
          ...
          html += `<tr><td>${i+1}</td><td><strong>${formatDistance(p.totalDist)}</strong></td><td>${p.hopCount}</td><td style="font-size:0.9em">${route}</td><td>${pktLink}</td><td>${viewPathBtn}</td></tr>`;
        });
        html += `</tbody></table></div>`;
      }
