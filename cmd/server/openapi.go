...
func routeDescriptions() map[string]routeMeta {
	return map[string]routeMeta{
		...
		"GET /api/analytics/relay-airtime-share": {Summary: "Relay airtime share per payload type", Description: "Overview tab's Relay Airtime Share. Each distinct packet (deduplicated by content hash) scores LoRa Time-on-Air of its raw bytes × distinct repeaters that relayed it, under the assumed preset returned in preset (config analytics.loraPreset); originator TX is excluded. rows[] = {payload_type, type, route_class, count, count_pct, score, airtime_pct}, sorted by airtime_pct desc. type is the numeric payload type; payload_type is a display label and may change. ADVERT is split into up to three rows that all have type 4: route_class \"flood\" (route_type 0/1, label \"ADVERT (flood)\"), \"zero_hop\" (route_type 2/3, label \"ADVERT (zero-hop)\") and \"legacy\" (NULL or out-of-range route_type, label \"ADVERT\"). route_class is null on every other row, so key rows by (type, route_class), never by type alone. The route is the one stored on the transmission: the first observation the ingestor inserted for that content hash, even if later observations of the same hash arrive on the other route class (e.g. a contact re-shared as a zero-hop advert). count_pct and airtime_pct are shares of total_count and total_score (nanoseconds). Payload Type Mix (/api/analytics/rf payloadTypes) is not split. Cached in the RF analytics cache (60s default).", Tag: "analytics",
			QueryParams: []paramMeta{
		...
		"POST /api/channel-proposals":                     {Summary: "Suggest a public hashtag channel", Description: "Body {name}. Only public hashtag channel names (at most 31 UTF-8 bytes including #, case preserved) are accepted — never keys. Returns 202 {requestId}; 400 invalid name, 403 disabled, 429 rate limited, 503 queue full (both with Retry-After).", Tag: "channels"},
		"GET /api/channel-proposals/requests/{requestId}": {Summary: "Status of a suggestion or review request", Description: "Returns {status: queued|pending|approved|rejected|error, proposal: {id, name, status, createdAt, reviewedAt}, error}. 404 when unknown or expired (24h).", Tag: "channels"},
		"GET /api/admin/channel-proposals":                {Summary: "List channel suggestions", Description: "Returns {proposals, enabled}, newest first, bounded. Optional status filter.", Tag: "admin", Auth: true, QueryParams: []paramMeta{{Name: "status", Description: "pending, approved, rejected or revoked", Type: "string"}}},
