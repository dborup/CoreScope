 ...
 *      (#1833 round 2 changed the anchor from `fixed` to `absolute`: the
 *      group must resolve its offsets against .live-page, the same box as
 *      .vcr-bar, or it drifts into the bar by --bottom-nav-reserve at
 *      641..768px and stops taking clicks.)
...
if (legendBtn) {
  assert(
    ...
    '.legend-toggle-btn uses position: absolute (docked to .live-page, #1833 round 2)'
  );
  assert(
...
if (feedShowBtn) {
  assert(
    ...
    '.feed-show-btn uses position: absolute (docked to .live-page, #1833 round 2)'
  );
  assert(
