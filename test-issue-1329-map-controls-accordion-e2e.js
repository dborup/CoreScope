...
async function run() {
  ...
  await step('desktop (1280px): panel position:absolute, all section contents visible', async () => {
    const data = await p2.evaluate(() => {
      ...
      // toggles, and #1771's "Important Links" rank-by select/slider, which
      // a checkbox reveals on demand). Those are hidden by an explicit
      // toggle, NOT by the mobile accordion this desktop check guards
      // against: the accordion collapses via the
      // `.mc-collapsed > *:not(legend){display:none}` CSS rule (mobile
      // media query, class-based, no inline style), so a desktop accordion
      // regression is still counted here and caught.
