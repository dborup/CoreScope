...
`blacklist-test.sh` covers both 10.1 and 10.2 in one run. Required env: `TEST_NODE_PUBKEY` (hex, of a real visible node on TARGET), `TARGET_SSH_HOST`, `TARGET_CONFIG_PATH`, `TARGET_CONTAINER`. §10.2 is SQLite-only and needs at least one db path, each used only by its own runner: `TARGET_CONTAINER_DB_PATH` (path inside the container, for `docker exec … sqlite3`) and/or `TARGET_HOST_DB_PATH` (path on the target host, for its own `sqlite3`). The two may differ; a path that is not a non-empty file in its runner's environment fails §10.2 without creating a file. `TARGET_DB_PATH` and `ADMIN_API_TOKEN` are no longer read and make the run refuse to start. Optional: `TARGET_SSH_KEY` (default `/root/.ssh/id_ed25519`), `CURL_TIMEOUT`, `RESTART_WAIT_S`. Mandatory teardown removes the pubkey and verifies the node returns to listings. The pubkey, SQL and URLs never appear in process arguments (stdin / mode-600 files instead).
